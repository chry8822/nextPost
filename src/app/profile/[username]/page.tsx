...
import { User, Calendar, Globe, Github, Linkedin, FileText, Heart, MessageCircle, ArrowLeft, LogOut } from 'lucide-react';
    ...
    return (
      <main className="container mx-auto px-4 py-8">
        <div className="text-center py-12">
          <h1 className="text-2xl font-bold text-slate-900  mb-4">{error || '사용자를 찾을 수 없습니다'}</h1>
    ...
    <main className="container mx-auto px-4 py-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <Card className="sticky top-8">
              <CardContent className="pt-6">
                <div className="text-center mb-6">
                  ...
                  />
                  <h1 className="text-2xl font-bold text-slate-900 mt-4">{user.name || user.username}</h1>
                  <p className="text-slate-500 ">@{user.username}</p>
                </div>

                {user.bio && (
                  <div className="mb-6">
                    <p className="text-slate-700  text-center">{user.bio}</p>
                  </div>
                )}
                ...
                <div className="space-y-3 mb-2">
                  ...
                  <div className="flex items-center">
                    <Globe className={`h-4 w-4 mr-2 ${user.website ? 'text-gray-600' : 'text-gray-400'}`} />
                    {user.website ? (
                      <a
                      ...
                      <span className="text-gray-400 text-sm">website.com/</span>
                    )}
                  </div>

                  <div className="flex items-center">
                    <Github className={`h-4 w-4 mr-2 ${user.github ? 'text-gray-600' : 'text-gray-400'}`} />
                    {user.github ? (
                      <a
                  ...
                  <div className="flex items-center">
                    <Linkedin className={`h-4 w-4 mr-2 ${user.linkedin ? 'text-gray-600' : 'text-gray-400'}`} />
                    {user.linkedin ? (
                      <a
          ...
          <Button variant="outline" size="sm" className=" px-1" onClick={handleSignOut}>
